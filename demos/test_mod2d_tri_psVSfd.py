...
## plot 2D data (uncomment first)
plt.imshow(data,aspect='auto',cmap="seismic", extent=[0,dx*(nx-1),dt*(nt-1),0]);
plt.gca().set_xlabel("X (m)",fontsize='large', fontweight='normal')
# plt.gca().set_ylabel("Y (m)",fontsize='large', fontweight='normal')
plt.gca().set_ylabel("Time (s)",fontsize='large', fontweight='normal')
plt.title('2D synthetic data (PS)')
plt.savefig(fname='figs/data2d-ps.png',format='png',dpi=300)
plt.show()

## plot 2D wavefields (uncomment first)
fignames=[]
...
# 	plot3d(wfd[:,:,:,ii],vmin=-1,vmax=1,z=np.arange(nz)*dz,x=np.arange(nx)*dz,y=np.arange(nz)*dz,barlabel='Amplitude',showf=False,close=False)
	...
	figname='figs/wfd2d-ps-%d.png'%ii;fignames.append(figname);
...
gengif(fignames,'gifs/wfd2ds-ps.gif')
...
# 	datas[:,inds,ii]=np.transpose(data[:,inds],(1, 0, 2))
	...
	# plt.gca().set_ylabel("Y (m)",fontsize='large', fontweight='normal')
	plt.gca().set_ylabel("Time (s)",fontsize='large', fontweight='normal')
	plt.title('2D synthetic data (PS) - %d'%(ii+1))
	plt.savefig(fname='figs/data2d-ps-%d.png'%(ii+1),format='png',dpi=300)
	plt.show()
...
plt.gca().set_xlabel("X (m)",fontsize='large', fontweight='normal')
plt.gca().set_ylabel("Z (m)",fontsize='large', fontweight='normal')
plt.title('Location image (PS)')
plt.colorbar(orientation='horizontal',shrink=0.6,label='Amplitude');
plt.savefig(fname='figs/location-ps.png',format='png',dpi=300)
plt.show()




## Below is for FD (almost the same image)
[data,wfd]=afd2d(vel,nt,dt,ax=[0,dx,nx],az=[0,dz,nz],ifsnaps=1,jsnap=4,verb=0,sx=[30,40],sz=[30,40],f=[10,10],t=[0.2,0.35],A=[1,1]);
...
plt.savefig(fname='figs/data2d-fd.png',format='png',dpi=300)
plt.show()
...
# 	plot3d(wfd[:,:,:,ii],vmin=-1,vmax=1,z=np.arange(nz)*dz,x=np.arange(nx)*dz,y=np.arange(nz)*dz,barlabel='Amplitude',showf=False,close=False)
	...
	figname='figs/wfd2d-fd-%d.png'%ii;fignames.append(figname);
	plt.savefig(fname=figname,format='png',dpi=300)

## plot 2D wavefield animation in GIF
from pyseistr import gengif #pip install git+https://github.com/aaspip/pyseistr
gengif(fignames,'gifs/wfd2ds-fd.gif')
...
# 	datas[:,inds,ii]=np.transpose(data[:,inds],(1, 0, 2))
	datas[:,inds,ii]=data[:,inds]
	[img,wfd]=afd2d(vel,nt,dt,ax=[0,dx,nx],az=[0,dz,nz],ifsnaps=1,jsnap=4,dat=datas[:,:,ii],tri=True);
	print('wfd max,min',wfd.max(),wfd.min())
	np.save('npys/wfd-tri-%d'%ii,wfd)
	plt.imshow(datas[:,:,ii],aspect='auto',cmap="seismic", extent=[0,dx*(nx-1),dt*(nt-1),0]);
	plt.gca().set_xlabel("X (m)",fontsize='large', fontweight='normal')
	# plt.gca().set_ylabel("Y (m)",fontsize='large', fontweight='normal')
	plt.gca().set_ylabel("Time (s)",fontsize='large', fontweight='normal')
	plt.title('2D synthetic data (FD) - %d'%(ii+1))
	plt.savefig(fname='figs/data2d-fd-%d.png'%(ii+1),format='png',dpi=300)
	plt.show()
...
plt.gca().set_xlabel("X (m)",fontsize='large', fontweight='normal')
plt.gca().set_ylabel("Z (m)",fontsize='large', fontweight='normal')
plt.title('Location image (FD)')
plt.colorbar(orientation='horizontal',shrink=0.6,label='Amplitude');
plt.savefig(fname='figs/location-fd.png',format='png',dpi=300)
plt.show()

from pyseistr import gengif
gengif(['figs/data2d-ps-1.png','figs/data2d-fd-1.png'],'gifs/data2d-psfd-1.gif')
gengif(['figs/data2d-ps.png','figs/data2d-fd.png'],'gifs/data2d-psfd.gif')

ii=100
gengif(['figs/wfd2d-ps-%d.png'%ii,'figs/wfd2d-fd-%d.png'%ii],'gifs/wfd2d-psfd-%d.gif'%ii)

## plot 2D TR wavefields (uncomment first)
# jj=0
