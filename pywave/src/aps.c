...
//       spz = np_floatalloc(ns);
    ...
    for (i=0; i<ns; i++)
    {
        ...
        spx[i]=tmp;
        tmp=*((float*)PyArray_GETPTR1(arrf2,ns+i));
        spy[i]=tmp;
        tmp=*((float*)PyArray_GETPTR1(arrf2,ns*2+i));
        spz[i]=tmp;
        f0[i]=*((float*)PyArray_GETPTR1(arrf2,ns*3+i));
        t0[i]=*((float*)PyArray_GETPTR1(arrf2,ns*4+i));
        A[i]=*((float*)PyArray_GETPTR1(arrf2,ns*5+i));
    }
...
static struct PyModuleDef ftfacfunModule = {
  ...
  "apscfun",  /*acoustic pseudo-spectral method*/
