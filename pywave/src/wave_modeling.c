...
void forward_modeling_a(np_file Fdat, np_mpi *mpipar, np_sou soupar, np_acqui acpar, np_vec array, bool verb)
...
// 	swap=fopen("temswap.bin", "wb+");
	...
	for(is=mpipar->cpuid; is<acpar->ns; is+=mpipar->numprocs){
		...
		for(it=0; it<nt; it++){
			if(verb) np_warning("Modeling is=%d; it=%d;", is+1, it);
